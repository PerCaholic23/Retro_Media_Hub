...
export default function Profile() {
  ...
  console.log("API Profile.jsx", API);
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API}/api/profile/${storedUser.id}`, {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${API}/api/profile/${storedUser.id}`, user, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="bg-[#e9eff3] min-h-screen font-prompt p-10 md:p-20 relative">
      <div className="bg-white rounded-3xl border-2 border-black p-8 md:p-12 shadow-lg max-w-6xl mx-auto relative overflow-hidden">
            ...
            <div className="pt-4">
              ...
              <div className="mb-4">
                {user.promptpayQR ? (
                  ...
                  <div className="w-40 h-40 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center text-gray-400 text-xs text-center px-2">ยังไม่ได้เพิ่มรูป QR Code</div>
                )}
              </div>
        ...
        {/* ===== ปุ่มประวัติการสั่งซื้อ (ซ้ายล่าง) ===== */}
        <div className="absolute bottom-12 left-8">
          <button 
            onClick={() => navigate("/order-history")}
            className="text-gray-600 font-bold border-2 border-gray-500 px-6 py-2 rounded-xl hover:bg-[#f28c45] hover:text-white transition-all flex items-center gap-2 shadow-sm"
          >
